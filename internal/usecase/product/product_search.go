...
// 	endpoint := "product/all-product?keyword="
// 	pipeline := mongo.Pipeline{}
...
func (s *ProductService) GetV2(
	...
	// ===== Execute Query =====
	// cur, err := s.db.Collection(models.Product{}.CollectionName()).Aggregate(ctx, pipeline)
	cur, err := s.db.Collection(models.Product{}.CollectionName()).
		Aggregate(ctx, pipeline, options.Aggregate().SetAllowDiskUse(true)) // เปิดใช้งาน external sorting
	if err != nil {
		helper.PrintStructJson(" ----- err Aggregate ----- ")
		helper.PrintStructJson(err)
	...
	response := map[string]interface{}{
		"total_data":  len(result), // จำนวนข้อมูลทั้งหมด
		"total_pages": 0,           // จำนวนหน้าทั้งหมด
		"data":        result,      // ข้อมูลที่กรองตาม filter
	}
